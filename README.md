...
- A push to the current default-branch head publishes only the stable full-SHA alias.
- An approved `vX.Y.Z` tag publishes stable full-SHA and semantic-version aliases plus the GitHub Release.
- Pull requests and manual validation runs never write images or releases.

The `dev` alias is intentionally mutable and must not be used for production deployment. OCI registries do not guarantee tag immutability, so production must use the tag-and-digest form documented below; the digest is the immutable image identity.
